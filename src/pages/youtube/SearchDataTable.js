...
const YoTubeDataTable = ({ data }) => {
  ...
  const handleExportCSV = () => {
      ...
      data.forEach((row) => {
        const rowData = [row.username, row.comment];
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ fontSize: 15 }}><b>Commenters</b></TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {paginatedData.map((row, index) => (
              <TableRow key={index}>
                ...
                <TableCell sx={{ fontSize: 15 }}>{row.comment}</TableCell>
                <TableCell onClick={() => window.open(`https://www.youtube.com/${row.username}`, "_blank")} style={{color: 'blue'}} sx={{ fontSize: 15 }}>{row.username}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
