...
const Dashboard = () => {
  ...
  const [totalDebtStock, setTotalDebtStock] = useState([]);

  useEffect(() => {
    getExternalDebtStock();
    getEducationExpenditure();
    getTotalDebtStock();
  }, []);

  //Get External Debt Stock each Country
  const getExternalDebtStock = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(API_URL + "debt-stock");
  ...
  const getEducationExpenditure = async () => {
    try {
      const response = await axios.get(API_URL + "education-expenditure");
  ...
  //Get total external debt stock from 2012-2022 year
  const getTotalDebtStock = async () => {
    try {
      const response = await axios.get(API_URL + "total/debit-stock");
      setTotalDebtStock(response.data);
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  };

  const labels = totalDebtStock.map((item) => item.country);
  const values = totalDebtStock.map((item) => item.totalDebtStock);
  ...
  const totalExternalDebt_options = {
    responsive: true,
    maintainAspectRatio: false,
    plugins: {
      title: {
        display: true,
        text: "Total External Debt Stock from 2012 - 2022",
      },
    },
  };
  ...
  return (
    <Fragment>
      ...
      <hr />
      <div style={{ width: "100%", height: "500px", paddingBottom: "4rem" }}>
        <Pie
          data={totalExternalDebt}
          options={totalExternalDebt_options}
          style={{}}
        />
      </div>
      <hr />
