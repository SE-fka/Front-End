...
const TelegramDataTable = ({ data }) => {
  ...
  const handleExportCSV = () => {
      let csvContent = 'channel_name,title,channel_url\n';
      data.forEach((row) => {
        const rowData = [row[0], row[1], row[2]];
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ fontSize: 15 }}><b>Title</b></TableCell>
              <TableCell sx={{ fontSize: 15 }}><b>Chanel Link</b></TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {paginatedData.map((row, index) => (
              <TableRow key={index}>
                ...
                <TableCell sx={{ fontSize: 15 }}>{row[1]}</TableCell>
                <TableCell onClick={() => window.open(`https://telegram.me/${row[2]}`, "_blank")} style={{color: 'blue'}} sx={{ fontSize: 15 }}>{row[2]}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
